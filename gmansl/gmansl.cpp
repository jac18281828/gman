...
RtBoolean compile(char* shader) {
  ...
  const std::filesystem::path sourcePath = std::filesystem::path(shader).replace_extension();
...
FILE* initHeader(const std::string& headerName) {
  ...
  if (headerFile) {
    fprintf(headerFile, "/* SPDX-License-Identifier: LGPL-2.1-or-later\n"
                        " *\n"
                        " * This is part of GMAN, a RenderMan-compatible renderer.\n"
                        " *\n"
    ...
    fprintf(headerFile,
            ...
            " */\n\n",
            slBaseName.c_str(), slBaseName.c_str(), slBaseName.c_str());
  }
  return headerFile;
}
