...
const RtFloat RGBYIQ[] = {0.299, 0.587, 0.144, 0.596, -0.275, -0.321, 0.212, -0.523, 0.311};

const RtFloat YIQRGB[] = {1.000, 0.956, 0.621, 1.000, -0.272, -0.647, 1.000, -1.105, 1.702};

const RtFloat RGBYUV[] = {0.299, 0.587, 0.114, -0.147, -0.289, 0.437, 0.615, -0.515, -0.100};

const RtFloat YUVRGB[] = {1.0, 0.0, 1.140, 1.0, -0.394, -0.581, 1.0, 2.028, 0.0};
...
static RtFloat subRGB(RtFloat a, RtFloat m1, RtFloat m2) {
  ...
  if (a < 60)
    return m1 * (m2 - m1) * a / 60;
  if (a < 180)
    return m2;
  if (a < 240)
    return m1 * (m2 - m1) * (240 - a) / 60;
  else
    return m1;
}
...
// ==== XYZ ==== //
RtVoid GMANConvertRGBtoXYZ(RtFloat* c) { convert(c, RGBXYZ); }
RtVoid GMANConvertXYZtoRGB(RtFloat* c) { convert(c, XYZRGB); }
...
// ==== YIQ ==== //
RtVoid GMANConvertRGBtoYIQ(RtFloat* c) { convert(c, RGBYIQ); };
RtVoid GMANConvertYIQtoRGB(RtFloat* c) { convert(c, YIQRGB); };

// ==== YUV ==== //
RtVoid GMANConvertRGBtoYUV(RtFloat* c) { convert(c, RGBYUV); };
RtVoid GMANConvertYUVtoRGB(RtFloat* c) { convert(c, YUVRGB); };
