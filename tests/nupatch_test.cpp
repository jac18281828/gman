...
// the expected sign.
bool normalsParallel(GMANVector const& a, GMANVector const& b, double tol, bool sameSign) {
  ...
  double dot = (a.getX() * b.getX() + a.getY() * b.getY() + a.getZ() * b.getZ()) / (amag * bmag);
...
std::vector<RtFloat> scaled(std::vector<RtFloat> const& base, double scale, int pntSize) {
...
void testBezierEquivalence() {
  ...
  for (double scale : kScales) {
    ...
    GMANNuPatch nupatch(4, 4, uknot, 0, 1, 4, 4, vknot, 0, 1, p.data(), false, pl);
    ...
    for (int iu = 0; iu <= 4; iu++) {
      for (int iv = 0; iv <= 4; iv++) {
        ...
        check(pointsNear(a, b, tol), "Bezier equivalence: getLocation(" + std::to_string(u) + "," + std::to_string(v) +
                                         ") matches GMANPatch at scale " + std::to_string(scale));
        ...
        if (scale == 1.0) {
          ...
          check(normalsParallel(na, nb, 1e-4, true),
                "Bezier equivalence: getNormal(" + std::to_string(u) + "," + std::to_string(v) + ") matches GMANPatch");
        }
      }
    }
  }
...
void testClosedRightEnd() {
  ...
  for (double scale : kScales) {
    ...
    check(pointsNear(got, last, tol), "Closed right end: getLocation(1,1) is the last control point "
                                      "at scale " +
                                          std::to_string(scale));
  }
}
...
void testRationalCylinder() {
  RtFloat uknot[12] = {0, 0, 0, 0.25f, 0.25f, 0.5f, 0.5f, 0.75f, 0.75f, 1, 1, 1};
  ...
  for (double scale : kScales) {
    ...
    GMANParameterList pl;
    GMANNuPatch cyl(9, 3, uknot, 0, 1, 2, 2, vknot, 0, 1, c.pw.data(), true, pl);

    const double us[] = {0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0};
    const double vs[] = {0.0, 0.5, 1.0};
    for (double u : us) {
      for (double v : vs) {
        GMANPoint loc = cyl.getLocation(u, v);
        const double dist = std::sqrt((double)loc.getX() * loc.getX() + (double)loc.getY() * loc.getY());
        check(std::fabs(dist - c.r) <= tol, "Rational cylinder: distance from z axis is r at u=" + std::to_string(u) +
                                                " v=" + std::to_string(v) + " scale=" + std::to_string(scale));
        check(std::fabs((double)loc.getZ() - c.h * v) <= tol, "Rational cylinder: z = h*v at u=" + std::to_string(u) +
                                                                  " v=" + std::to_string(v) +
        ...
        check(normalsParallel(n, expected, 1e-4, true),
              "Rational cylinder: normal is radial, outward, at u=" + std::to_string(u) + " v=" + std::to_string(v) +
                  " scale=" + std::to_string(scale));
      }
    }
  }
...
void testSubRange() {
  ...
  for (double scale : kScales) {
    ...
    GMANParameterList pl;
    GMANNuPatch full(9, 3, uknot, 0, 1, 2, 2, vknot, 0, 1, c.pw.data(), true, pl);
    GMANNuPatch sub(9, 3, uknot, 0.25, 0.75, 2, 2, vknot, 0, 1, c.pw.data(), true, pl);

    const double vs[] = {0.0, 0.5, 1.0};
    for (double v : vs) {
      check(pointsNear(sub.getLocation(0.0, v), full.getLocation(0.25, v), tol),
            "Sub-range: getLocation(0,v) matches full range at u=0.25, "
            "v=" +
                std::to_string(v) + " scale=" + std::to_string(scale));
      check(pointsNear(sub.getLocation(1.0, v), full.getLocation(0.75, v), tol),
...
void testRenderTwin(std::string const& gman, std::string const& ribDir) {
  ...
  check(runGman(gman, ribDir + "/patch_bilinear_twin.rib") == 0, "patch_bilinear_twin.rib renders");
  checkGoldenImage("nupatch_bilinear.tif", "patch_bilinear_twin.tif", GOLDEN_CHANNEL_TOL, GOLDEN_MAX_FRACTION,
...
void testTextureCoordinatesNoOp(std::string const& gman, std::string const& ribDir) {
  check(writeCheckerTexture("checker_texture.tif"), "checker_texture.tif writes into the render's working directory");
  check(runGman(gman, ribDir + "/nupatch_texturecoordinates.rib") == 0, "nupatch_texturecoordinates.rib renders");
  check(runGman(gman, ribDir + "/nupatch_texturecoordinates_default.rib") == 0,
        "nupatch_texturecoordinates_default.rib renders");

  GmanImage withCoords = readGmanTIFF("nupatch_texturecoordinates.tif");
  GmanImage defaultCoords = readGmanTIFF("nupatch_texturecoordinates_default.tif");
  check(withCoords.ok && defaultCoords.ok, "both renders read back");
  check(pixelIdentical(withCoords, defaultCoords), "TextureCoordinates never changes a NuPatch's render: "
  ...
  if (defaultCoords.ok && defaultCoords.width > 110 && defaultCoords.height > 110) {
    ...
    check(dr > GOLDEN_CHANNEL_TOL || dg > GOLDEN_CHANNEL_TOL || db > GOLDEN_CHANNEL_TOL,
...
void testMalformedFixtures(std::string const& gman, std::string const& malformedDir) {
  ...
  const Fixture fixtures[] = {
      {"nupatch_short_uknot.rib", "NuPatch: uknot length 3 does not match nu + uorder = 4; ignoring."},
      {"nupatch_decreasing_knots.rib", "NuPatch: uknot[2]=0.25 is less than uknot[1]=0.5, not "
                                       "non-decreasing; ignoring."},
      {"nupatch_order_exceeds_n.rib", "NuPatch: nu=2 uorder=3 violates nu >= uorder >= 1; ignoring."},
      {"nupatch_empty_range.rib", "NuPatch: umin=0.5 umax=0.5 violates umin < umax; ignoring."},
      {"nupatch_umin_below_knot.rib", "NuPatch: umin=-0.1 is less than uknot[uorder-1]=0; ignoring."},
      {"nupatch_short_pw.rib", "Parameter \"Pw\": declared length 16, supplied length 14; "
