...
void testAsymmetricPartialSphere(const std::string& gman, const std::string& ribDir) {
  const std::string rib = ribDir + "/partial_sphere.rib";
  check(runGman(gman, rib) == 0, "partial_sphere.rib renders (default Clipping would corrupt this "
  ...
  check(differsFromBackground(partial, cx, cy), "partial sphere: the very centre (the surviving cap near zmax's "
  ...
  // zmin=-0.5 opens up.
  check(!differsFromBackground(partial, cx + 20, cy), "partial sphere: a mid-radius sample (r=20px) is background -- "
                                                      "inside the gap the missing near cap leaves");
  check(!differsFromBackground(partial, cx + 35, cy), "partial sphere: a mid-radius sample (r=35px) is background -- "
                                                      "still inside the gap");

  // Near the true outer edge, the thin surviving ring should still show.
  check(differsFromBackground(partial, cx + 49, cy), "partial sphere: a near-edge sample (r=49px) is lit -- the thin "
  ...
  const std::string fullRib = "Display \"partial_sphere_full_control.tif\" \"file\" \"rgba\"\n"
                              "Format 200 200 1\n"
  ...
  check(runGman(gman, "partial_sphere_full_control.rib") == 0, "full-sphere control scene renders");
  ...
  check(differsFromBackground(full, cx + 20, cy) && differsFromBackground(full, cx + 35, cy),
