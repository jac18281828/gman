...
void checkFaceGeometry(const std::string& label, GMANBody* body, double expectedArea,
  ...
  check(std::fabs(summedArea - expectedArea) <= 1e-6 * std::max(1.0, expectedArea),
        label + ": area matches its own face (got " + std::to_string(summedArea) + ", expected " +
...
void testOneFaceIdentity() {
  ...
  for (const auto& namedRing : cases) {
    ...
    check(std::fabs(polyArea - pointsArea) <= 1e-9 * std::max(1.0, polyArea), name + ": same area");
...
void testIndicesNotOrder() {
  const std::vector<GMANPoint> canonical = {{0, 0, 0}, {1, 0, 0}, {2, 0, 0}, {0, 1, 0}, {1, 1, 0}, {2, 1, 0}};
  ...
  std::vector<RtInt> verts = {newIndexOf[0], newIndexOf[1], newIndexOf[4], newIndexOf[3],
                              newIndexOf[1], newIndexOf[2], newIndexOf[5], newIndexOf[4]};

  GMANPrimitive* prim = runGetRSPointsPolygon(2, {4, 4}, verts, scrambled);
  ...
  check(bodyA != nullptr && bodyB != nullptr, "indices, not order: both bodies present");
  if (bodyA != nullptr) {
    checkFaceGeometry("indices, not order: face A", bodyA, ringArea(faceARing), newellNormal(faceARing));
  }
  if (bodyB != nullptr) {
    checkFaceGeometry("indices, not order: face B", bodyB, ringArea(faceBRing), newellNormal(faceBRing));
  }
...
void testHoleThroughIndices() {
  ...
  GMANPrimitive* pointsPrim = runGetRSPointsGeneralPolygons(1, {2}, {4, 4}, {0, 1, 2, 3, 4, 5, 6, 7}, points);
  ...
  check(pointsObj != nullptr && generalObj != nullptr, "hole through indices: both return an object");
...
void testOneBadFace() {
  ...
  if (object != nullptr) {
    ...
    if (body0 != nullptr) {
      checkFaceGeometry("one bad face: face 0", body0, ringArea(face0Ring), newellNormal(face0Ring));
    }
    if (body2 != nullptr) {
      checkFaceGeometry("one bad face: face 2", body2, ringArea(face2Ring), newellNormal(face2Ring));
    }
  }
  delete prim;
  ...
  GMANPrimitive* degeneratePrim = runGetRSPointsPolygon(3, nverts, verts, allDegenerate);
  GMANObject* degenerateObject = dynamic_cast<GMANObject*>(degeneratePrim);
  check(degenerateObject != nullptr, "one bad face, all degenerate: returns an object");
...
void testUnreferencedPoints() {
  ...
  check(baseObj != nullptr && extraObj != nullptr, "unreferenced points: both return an object");
...
void testMalformedFixtures(const std::string& gman, const std::string& malformedDir) {
  ...
  const Fixture fixtures[] = {
      {"pointspolygons_short_verts.rib", "PointsPolygons: verts length 7 does not match nverts sum 8; "
                                         "ignoring."},
      {"pointsgeneralpolygons_short_nverts.rib", "PointsGeneralPolygons: nverts length 1 does not match nloops sum "
                                                 "2; ignoring."},
  ...
  for (const Fixture& fixture : fixtures) {
    ...
    check(r.exitStatus == fixture.expectedExit,
          std::string(fixture.file) + ": exits " + std::to_string(fixture.expectedExit));
...
int main(int argc, char* argv[]) {
  ...
  renderAndCompare(gman, ribDir, "pointspolygons_textured", "polygons_textured");
  renderAndCompare(gman, ribDir, "pointsgeneralpolygons_hole", "generalpolygon_hole_twin");
