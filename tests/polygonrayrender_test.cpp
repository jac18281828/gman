...
int main(int argc, char* argv[]) {
  ...
  check(status == 0, "r5a_polygon.rib renders under -r gmanraytracer (exit " + std::to_string(status) + ")");

  checkGoldenImage("r5a_polygon.tif", ribDir + "/r5a_polygon_golden.tif", GOLDEN_CHANNEL_TOL, GOLDEN_MAX_FRACTION,
                   "r5a_polygon_diff.tif");

  return checkSummary("R5a: gman -r gmanraytracer renders a camera-facing pentagon");
}
