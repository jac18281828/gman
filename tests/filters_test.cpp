...
void testTriangleFilter() {
  // Peak at the origin.
  check(near(RiTriangleFilter(0.0, 0.0, 2.0, 2.0), 1.0), "triangle: peak 1.0 at the origin");

  // Symmetric in x and y.
  check(near(RiTriangleFilter(0.5, 0.3, 2.0, 2.0), RiTriangleFilter(-0.5, 0.3, 2.0, 2.0)), "triangle: symmetric in x");
  check(near(RiTriangleFilter(0.5, 0.3, 2.0, 2.0), RiTriangleFilter(0.5, -0.3, 2.0, 2.0)), "triangle: symmetric in y");

  // Zero at the boundary, and zero -- not the unclamped negative or
  // sign-flipped-positive value -- everywhere outside it.
  check(near(RiTriangleFilter(1.0, 1.0, 2.0, 2.0), 0.0), "triangle: zero at the boundary");
  check(near(RiTriangleFilter(1.5, 1.5, 2.0, 2.0), 0.0), "triangle: guard zeroes an off-axis point outside support "
                                                         "(unguarded this evaluates to +0.25)");
  check(near(RiTriangleFilter(1.5, 0.0, 2.0, 2.0), 0.0), "triangle: guard zeroes an on-axis point outside support "
  ...
  check(near(RiTriangleFilter(2.0, 0.0, 4.0, 2.0), 0.0), "triangle: zero exactly at the x-boundary when xwidth != 2.0");
  check(near(RiTriangleFilter(0.5, 1.5, 4.0, 2.0), 0.0), "triangle: guard fires from y alone exceeding its narrower "
                                                         "half-width while x stays in support "
                                                         "(unguarded this evaluates to -0.375)");
  check(near(RiTriangleFilter(1.5, 0.0, 8.0, 2.0), 0.625), "triangle: in-support point survives a wide xwidth that a "
  ...
  check(RiTriangleFilter(0.2, 0.0, 2.0, 2.0) > RiTriangleFilter(0.6, 0.0, 2.0, 2.0),
        "triangle: monotone decay along x");
  check(RiTriangleFilter(0.6, 0.0, 2.0, 2.0) > RiTriangleFilter(0.9, 0.0, 2.0, 2.0),
        "triangle: monotone decay continues toward the boundary");
}

void testGaussianFilter() {
  // Peak at the origin.
  check(near(RiGaussianFilter(0.0, 0.0, 2.0, 2.0), 1.0), "gaussian: peak 1.0 at the origin");

  // Symmetric in x and y.
  check(near(RiGaussianFilter(0.7, 0.4, 2.0, 2.0), RiGaussianFilter(-0.7, 0.4, 2.0, 2.0)), "gaussian: symmetric in x");
  check(near(RiGaussianFilter(0.7, 0.4, 2.0, 2.0), RiGaussianFilter(0.7, -0.4, 2.0, 2.0)), "gaussian: symmetric in y");

  // The scaled exponent, exp(-8*x^2/xwidth^2) at width 2: at x=1,
  // width=2, exp(-8*0.25) = exp(-2) = 0.1353 (the unscaled form gives
  // exp(-0.5) = 0.6065).
  check(near(RiGaussianFilter(1.0, 0.0, 2.0, 2.0), 0.1353), "gaussian: edge sample matches the RISpec-scaled reference "
  ...
  // pass by coincidence.
  check(near(RiGaussianFilter(2.0, 0.0, 4.0, 4.0), 0.1353), "gaussian: doubling x and xwidth together reproduces the "
                                                            "width-2 edge value");

  // Monotone decay from the centre out.
  check(RiGaussianFilter(0.2, 0.0, 2.0, 2.0) > RiGaussianFilter(0.6, 0.0, 2.0, 2.0),
        "gaussian: monotone decay along x");
  check(RiGaussianFilter(0.6, 0.0, 2.0, 2.0) > RiGaussianFilter(1.0, 0.0, 2.0, 2.0),
...
void testCatmullRomFilter() {
  ...
  check(near(RiCatmullRomFilter(0.0, 0.0, 2.0, 2.0), 1.0), "catmull-rom: peak 1.0 at the origin");

  check(near(RiCatmullRomFilter(0.5, 0.3, 2.0, 2.0), RiCatmullRomFilter(-0.5, 0.3, 2.0, 2.0)),
  ...
  // and gaussian.
  check(near(RiCatmullRomFilter(0.5, 0.0, 2.0, 2.0), RiCatmullRomFilter(0.5, 0.0, 8.0, 8.0)),
        "catmull-rom: ignores xwidth/ywidth, as found");

  // Zero outside |x|>=2 and |y|>=2.
  check(near(RiCatmullRomFilter(2.0, 0.0, 2.0, 2.0), 0.0), "catmull-rom: zero at x=2");
  check(near(RiCatmullRomFilter(3.0, 0.0, 2.0, 2.0), 0.0), "catmull-rom: zero beyond x=2");

  // Genuine negative side lobe -- not monotone, pinned as found rather
  // than asserted monotone (which would be false).
  check(RiCatmullRomFilter(1.5, 0.0, 2.0, 2.0) < 0.0, "catmull-rom: negative side lobe at x=1.5");
  check(near(RiCatmullRomFilter(1.5, 0.0, 2.0, 2.0), -0.0625), "catmull-rom: side lobe value at x=1.5 is -0.0625");
  check(RiCatmullRomFilter(2.0, 0.0, 2.0, 2.0) > RiCatmullRomFilter(1.5, 0.0, 2.0, 2.0),
        "catmull-rom: rises again from the side lobe toward x=2");
}

void testSincFilter() {
  // Pinned as found: infinite support by construction, no guard.
  check(near(RiSincFilter(0.0, 0.0, 2.0, 2.0), 1.0), "sinc: peak 1.0 at the origin");

  check(near(RiSincFilter(0.5, 0.3, 2.0, 2.0), RiSincFilter(-0.5, 0.3, 2.0, 2.0)), "sinc: symmetric in x");
  check(near(RiSincFilter(0.5, 0.3, 2.0, 2.0), RiSincFilter(0.5, -0.3, 2.0, 2.0)), "sinc: symmetric in y");
  ...
  check(near(RiSincFilter(10.5, 0.0, 2.0, 2.0), 0.0303), "sinc: nonzero far outside width 2, unguarded by design");

  // Genuine negative side lobe between x=1 and x=2, not monotone.
  check(RiSincFilter(1.5, 0.0, 2.0, 2.0) < 0.0, "sinc: negative side lobe at x=1.5");
