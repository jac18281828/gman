...
int main() {
  ...
  check(dictionary.getClass(id) == GMANTokenEntry::FACEVARYING, "facevarying float foo declares class FACEVARYING");
  check(dictionary.getType(id) == GMANTokenEntry::FLOAT, "facevarying float foo declares type FLOAT");
  ...
  check(size == facevarying, "allocSize returns the facevarying total, not the varying count");
  ...
  const int varyingSize = dictionary.allocSize(varyingId, vertex, varying, uniform, facevarying);
  check(varyingSize == varying, "a plain varying token still allocates by the varying count");
  ...
  check(dictionary.allocSize(vertexId, vertex, varying, uniform, facevarying) == vertex,
        "a vertex token allocates by the vertex count");

  GMANTokenId uniformId = dictionary.getTokenId("uniform float qux");
  check(dictionary.allocSize(uniformId, vertex, varying, uniform, facevarying) == uniform,
        "a uniform token allocates by the uniform count");

  GMANTokenId constantId = dictionary.getTokenId("constant float quux");
  check(dictionary.allocSize(constantId, vertex, varying, uniform, facevarying) == 1,
  ...
  check(dictionary.allocSize(constantPointId, vertex, varying, uniform, facevarying) == 3,
