...
std::string edgeRib(const std::string& display, const std::string& filterLine, const std::string& samplesLine) {
...
void testSupersamplingEdge(const std::string& gman) {
  writeFile("edge_1x1.rib", edgeRib("edge_1x1.tif", "PixelFilter \"box\" 1 1\n", "PixelSamples 1 1\n"));
  check(runGman(gman, "edge_1x1.rib") == 0, "1x1 edge scene renders");

  writeFile("edge_4x4.rib", edgeRib("edge_4x4.tif", "PixelFilter \"box\" 1 1\n", "PixelSamples 4 4\n"));
  ...
  check(!oneSampleIntermediate, "1x1: the edge's scanline has only fully-covered or "
  ...
  check(fourSampleIntermediate, "4x4: the same edge's scanline has at least one intermediate "
...
void testPixelFilterDiffers(const std::string& gman) {
  ...
  // (box at width 1 against Gaussian at width 2, say) can pass from the
  // width difference alone, without the filter function itself ever
  // being consulted.
  writeFile("edge_filter_box.rib", edgeRib("edge_filter_box.tif", "PixelFilter \"box\" 2 2\n", "PixelSamples 4 4\n"));
  ...
  check(runGman(gman, "edge_filter_gaussian.rib") == 0, "gaussian-filter scene renders");
  ...
  for (uint32_t x = 0; x < box.width; ++x) {
    int d = std::abs((int)TIFFGetR(box.at(x, y)) - (int)TIFFGetR(gaussian.at(x, y)));
    if (d > 4) {
      differs = true;
      break;
    }
  }
  check(differs, "box and Gaussian filters over the same samples produce "
...
void testSampleCountCeiling(const std::string& gman) {
  writeFile("sample_ceiling.rib", "Display \"sample_ceiling.tif\" \"file\" \"rgba\"\n"
                                  "Format 4096 2160 1\n"
                                  "PixelSamples 16 16\n"
                                  "WorldBegin\n"
                                  "WorldEnd\n");
