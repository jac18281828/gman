...
std::vector<GMANPoint> placeLoop(const std::vector<GMANPoint> &canonical,
...
std::vector<std::vector<GMANPoint>> placeLoops(
    const std::vector<std::vector<GMANPoint>> &canonical, double scale,
    bool rotated) {
...
GMANPrimitive *runGetRSGeneralPolygon(
...
std::vector<std::array<int, 3>> checkPlacement(
    const std::string &label, const std::vector<std::vector<GMANPoint>> &loops,
  ...
  check(faces == expectedFaces,
        label + ": " + std::to_string(chainLength) + " kept vertices, " +
            std::to_string(keptHoleCount) + " kept holes yield " +
            std::to_string(expectedFaces) + " triangles (got " +
            std::to_string(faces) + ")");
  ...
  for (GMANFace *face = surface ? surface->getFace() : nullptr; face != nullptr;
    ...
    double sign = crossX * outerNormal[0] + crossY * outerNormal[1] +
                  crossZ * outerNormal[2];
  ...
  check(orientationOk,
        label +
            ": every non-exempt triangle agrees with the outer loop's "
            "orientation");
  check(coverageOk, label +
                        ": every vertex on the object's chain appears in a "
                        "triangle");
...
void runCase(const std::string &name,
             const std::vector<std::vector<GMANPoint>> &canonical,
  ...
  for (double scale : kScales) {
    for (bool rotated : {false, true}) {
      ...
      std::vector<std::vector<GMANPoint>> loops =
          placeLoops(canonical, scale, rotated);
      std::vector<std::array<int, 3>> triples =
          checkPlacement(buf, loops, kept);
...
std::vector<GMANPoint> orderOuter() {
  return {{0, 0, 0}, {8, 0, 0}, {8.5f, 2, 0}, {8, 4, 0}, {0, 4, 0}};
}
std::vector<GMANPoint> orderHoleLeft() {
  return {{3, 2, 0}, {2, 1, 0}, {1, 2, 0}, {2, 3, 0}};
}
std::vector<GMANPoint> orderHoleRight() {
  return {{7, 2.3f, 0}, {6, 1.3f, 0}, {5, 2.3f, 0}, {6, 3.3f, 0}};
}
...
std::vector<GMANPoint> occludedOuter() {
  return {{0, 0, 0}, {6, 0, 0},    {7, 6, 0}, {5, 6, 0},
          {5, 3, 0}, {4.5f, 6, 0}, {0, 6, 0}};
}
std::vector<GMANPoint> occludedHole() {
  return {{2, 2, 0}, {1.5f, 1.5f, 0}, {1, 2, 0}, {1.5f, 2.5f, 0}};
}
...
// lobe.
std::vector<GMANPoint> sharedApexOuter() {
  return {{0, 0, 0}, {4, 0, 0}, {4.5f, 2, 0}, {4, 4, 0}, {0, 4, 0}};
}
std::vector<GMANPoint> sharedApexHoleLower() {
  return {{3.1f, 1, 0}, {2.5f, 0.5f, 0}, {2, 1, 0}, {2.5f, 1.5f, 0}};
}
std::vector<GMANPoint> sharedApexHoleUpper() {
  return {{3, 3, 0}, {2.5f, 2.5f, 0}, {2, 3, 0}, {2.5f, 3.5f, 0}};
}

void runTrapRings() {
  runCase("order", {orderOuter(), orderHoleLeft(), orderHoleRight()},
          {true, true, true}, /*assertTripleIdentity=*/true);
  runCase("occluded", {occludedOuter(), occludedHole()}, {true, true},
          /*assertTripleIdentity=*/true);
  runCase("shared apex",
          {sharedApexOuter(), sharedApexHoleLower(), sharedApexHoleUpper()},
          {true, true, true}, /*assertTripleIdentity=*/true);
}

void runOneLoopParity(const std::string &name,
                      const std::vector<GMANPoint> &canonical) {
  for (double scale : kScales) {
    for (bool rotated : {false, true}) {
      char buf[256];
      std::snprintf(buf, sizeof(buf), "%s one-loop parity (scale=%.0e, %s)",
                    name.c_str(), scale, rotated ? "rotated" : "axis-aligned");
      ...
      GMANPrimitive *polyPrim =
          mgr.getRSPolygon(nverts, pl1, &options, &attr, &transform);
      GMANPrimitive *generalPrim =
          mgr.getRSGeneralPolygon(1, &nverts, pl2, &options, &attr, &transform);
      GMANObject *polyObj = dynamic_cast<GMANObject *>(polyPrim);
      GMANObject *generalObj = dynamic_cast<GMANObject *>(generalPrim);

      check(polyObj != nullptr && generalObj != nullptr,
            std::string(buf) + ": both return an object");
      ...
      for (GMANVertex *v = generalObj->getVert(); v != nullptr;
           v = v->getNext()) {
        generalVerts.push_back(v);
      }
      check(polyVerts.size() == generalVerts.size(),
            std::string(buf) + ": same vertex count");
      ...
      check(sameLocations, std::string(buf) +
                               ": same vertex locations, "
                               "same order");
      ...
      GMANSurface *polySurface =
          polyObj->getBody() ? polyObj->getBody()->getSurface() : nullptr;
      for (GMANFace *f = polySurface ? polySurface->getFace() : nullptr;
           f != nullptr; f = f->getNext()) {
        polyTriples.push_back({polyIndex[f->getVertex(0)],
                               polyIndex[f->getVertex(1)],
                               polyIndex[f->getVertex(2)]});
      }
      GMANSurface *generalSurface =
          generalObj->getBody() ? generalObj->getBody()->getSurface() : nullptr;
      for (GMANFace *f = generalSurface ? generalSurface->getFace() : nullptr;
           f != nullptr; f = f->getNext()) {
        generalTriples.push_back({generalIndex[f->getVertex(0)],
                                  generalIndex[f->getVertex(1)],
                                  generalIndex[f->getVertex(2)]});
      }
      check(polyTriples == generalTriples,
            std::string(buf) + ": same index triples as getRSPolygon");
...
std::vector<GMANPoint> squareOuter() {
  return {{0, 0, 0}, {4, 0, 0}, {4, 4, 0}, {0, 4, 0}};
}
std::vector<GMANPoint> squareHoleOrderA() {
  return {{1, 1, 0}, {3, 1, 0}, {3, 3, 0}, {1, 3, 0}};
}
...
void runSquareHoleBothWindings() {
  runCase("square with a square hole", {squareOuter(), squareHoleOrderA()},
          {true, true}, /*assertTripleIdentity=*/false);
  runCase("square with a hole wound the other way",
          {squareOuter(), squareHoleOrderB()}, {true, true},
...
void runDegenerateOuterLoop() {
  ...
  check(object != nullptr,
        "degenerate outer loop: getRSGeneralPolygon returns an object");
